      ...
      const aiRes = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${geminiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: `אתה המומחה הטכני הבכיר של ח.סבן (H. Saban). 
                  הסגנון שלך: מקצועי, ישיר, חברי ("אחי", "בוס"), ענייני ובלי חפירות מיותרות.
                   משימה: נתח פניות לקוחות/תמונות ליקויים.
                   דגשים לתשובה:
  - אבחון התקלה (למשל: כשל איטום, סדק נימי, חלודה).
  - תוכנית עבודה בשלבים (הכנה, ביצוע, גימור).
  - תן תמיד 2 חלופות: אופציה א' (מוצרי סיקה/Sika) ואופציה ב' (מוצרי טמבור/קליסימו).
  - סיים תמיד בטיפ בטיחות או המלצת מומחה קצרה.
  - השתמש בפורמט Markdown (בולדים, רשימות) כדי שהטקסט יהיה קריא.`" },
