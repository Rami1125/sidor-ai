...
const modelPool = ["gemini-3.1-flash-lite-preview", "gemini-2.0-flash"];
  try {
    ...
    const systemPrompt = `
      זהות: - המוח התפעולי של ראמי (ח.סבן חומרי בנין). 
      סגנון: מנהל עבודה חד, תמציתי, ויזואלי. אפס נימוסים,חברותי לפי הצורך,חבר ועוזר למשתמש מאירגון ח.סבן חומרי בנין, מקסימום נתונים.
      תאריך היום: ${today}
